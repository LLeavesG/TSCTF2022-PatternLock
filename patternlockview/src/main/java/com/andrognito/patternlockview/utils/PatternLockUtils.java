...
public class PatternLockUtils {
    ...
    public static String patternToString(PatternLockView patternLockView,
        ...
        int DotCount = patternLockView.getDotCount();
        StringBuilder result = new StringBuilder();
        String coding = "fedcba9876543210";
        if(DotCount <= 6){
            for (int i = 0; i < patternSize; i++) {
                PatternLockView.Dot dot = pattern.get(i);
                result.append(coding.charAt(dot.getRow() * patternLockView.getDotCount() + dot.getColumn()));
            }
        }else {
            for (int i = 0; i < patternSize; i++) {
                PatternLockView.Dot dot = pattern.get(i);
                result.append(dot.getRow() * patternLockView.getDotCount() + dot.getColumn()).append("#");
            }
        }
        int i = 0;
        for(i = 0; i < result.length(); i++){
            result.setCharAt(i, (char) (result.charAt(i) ^ 0x22));
        }
